...
class FetchLoopInternal<T = any> {
  ...
  get refreshInterval(): number {
    ...
    const vals = Array.from(listeners.values());
    const intervals = vals.map((listener) => listener.refreshInterval);
  ...
  get refreshIntervalOnError(): number | null {
    const listeners = this!.listeners;
    const vals = Array.from(listeners.values());
    const refreshIntervalsOnError: number[] = [...vals]
...
export function refreshAllCaches(): void {
  for (const loop of Array.from(globalLoops.loops.values())) {
    loop.refresh();
  }
}
